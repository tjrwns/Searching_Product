/*/
http://localhost:49781/ui/Page/_Component/DHTMLX/5.1.0/Windows/index.html
http://dnsproxy.shop:49781/ui/Page/_Component/DHTMLX/5.1.0/Windows/index.html

http://localhost:49781/ui/Page/_Component/DHTMLX/5.1.0/Windows/index.html?url=http://localhost:49781/ui/Page/_Component/DHTMLX/5.1.0/Windows/index.json
http://dnsproxy.shop:49781/ui/Page/_Component/DHTMLX/5.1.0/Windows/index.html?url=http://dnsproxy.shop:49781/ui/Page/_Component/DHTMLX/5.1.0/Windows/index.json
//*/
//----------------------------------------------------------------------------------------------------;
var fileNm = "ui/Page/_Component/DHTMLX/5.1.0/Windows/index.js";
...
(function(){
	...
	//JS;
	window.b2link.util.importJSsToHead( HOST + "/libs/dhtmlx/5.1.0/dhtmlxwindows-controller.js"
		, HOST + "/libs/dhtmlx/5.1.0/dhtmlxCommon/dhtmlxcommon-all.js"
...
var _IDS = (function(){
	return {
		ROOT : "ui/Page/_Component/DHTMLX/5.1.0/Windows/index"
	};
})();
...
var _DATA = (function(){
	...
	return {
		DHTMLX : {
			contents : {
				a : {
					...
					, renderer : function( win ){
						...
						win.attachObject( el );
						elc.setParentElement( win.cell.children[ 0 ] );
					}
			...
			//, contents : { a : "<div>HELLO</div>", b : "<div>HELLO</div>" }
		}
	};
	////[ SAMPLE ] - [ E ] - 자동 삭제;
})();
...
_evt_resize.resize.dhtmlxWindowCells = function(){
	var io;
 ...
 * dhtmlXWindows 활성화 시
 * @function
 * @param {dhtmlXWindowsCell} win
 */
var _evt_onFocus__dhx$Windows = function( win )
{
	_THIS.currentWindow = win;
};
 ...
 * @param {dhtmlXWindowsCell} win
 */
var _evt_onClose__dhx$Windows = function( win )
{
	var key = win._idd;
	win.hide();
	win._unload();
	//win.cell.parentElement.remove();
	win = null;
	delete _DHTMLX.windows[ key ];
};

/**
 * dhtmlXWindows resize 시
 * @function
 * @param {dhtmlXWindowsCell} win
 */
var _evt_onResizeFinish__dhx$Windows = function( win )
{
	//var el = window.b2link.element.getElementByClassName( win, "dhx_cell_cont_wins" );
	var el = win.cell.children[ 0 ];
	window.b2link.event.dispatchCE__RESIZED_EL( el, win );
};
...
function initialize()
{
	try
	{
		...
		else if( !initialize._bInit && window.dhtmlXWindows )
		{
...
_addWindow._fn0 = function( data, options ){
	...
	if( data.DHTMLX )
	{
		if( data.DHTMLX.contents )
		{
			...
			for( var s in T )
			{
				...
				if( "string" == typeof( io ) )//콘텐츠 내용이 HTML String인 경우;
				{
					try
					{
						...
						windowObject.attachHTMLString( io );
				...
				else if( "string" == typeof( io.text ) )//콘텐츠 내용이 HTML String이고 이외의 조건이 존재하는 경우;
				{
					...
					{
						windowObject = _addWindow._fn1( s, io, options );
						windowObject.attachHTMLString( io.text );
						_DHTMLX.windows[ s ] = windowObject;
					}
					catch( er )
					{
			...
			//_DHTMLX.layout.cells의 resize 이벤트 발생 시키기;
			_evt_resize.resize.dhtmlxWindowCells();
		}
...
_addWindow._fn1 = function( id, data, options ){
	...
	var windowObject = _DHTMLX._dhx$Windows.createWindow( id, o_param.x, o_param.y, o_param.width, o_param.height );
		windowObject.setText( o_param.title );//타이틀 설정;
	return windowObject;
};
_addWindow._fn1.param = {};

/**
 * 선택 윈도우를 최상단(활성화 상태)로 변경한다.
 * @function
 * @param {dhtmlXWindowsCell} windowObject
 */
var _bringToTop = function( windowObject )
{
	windowObject.bringToTop();
};
...
var _dispose = function()
{
	...
	if( _DHTMLX._dhx$Windows != null && _DHTMLX._dhx$Windows.unload != null )
	{
		for( var key in _DHTMLX.windows ) _DHTMLX.windows[ key ].close();
 ...
 * </code>
 * @param {null|Object} options DHTMLX Windowt의 options
 * <code>
 	{
		DHTMLX : {
			  width : 500
...
var _draw = function( data, options )
{
	...
	{
		//그리기 위한 로직 추가;
		_DHTMLX._dhx$Windows = new window.dhtmlXWindows();
		...
		_DHTMLX._dhx$Windows.attachEvent( "onClose", _evt_onClose__dhx$Windows );//window onClose;
		_DHTMLX._dhx$Windows.attachEvent( "onResizeFinish", _evt_onResizeFinish__dhx$Windows );//window onResizeFinish;
 ...
 * @function
 */
var _parkAll = function()
{
	for( var key in _DHTMLX.windows ) _DHTMLX.windows[ key ].park();
};
 ...
 * @function
 */
var _maximizeAll = function()
{
	for( var key in _DHTMLX.windows ) _DHTMLX.windows[ key ].maximize();
};
 ...
 * @function
 */
var _minimizeAll = function()
{
	var windowObject;
	for( var key in _DHTMLX.windows )
	{
		windowObject = _DHTMLX.windows[ key ];
		if( windowObject.isMaximized() ) windowObject.minimize();
		else
		{
			console.log( "_DHTMLX.windows[ " + key + " ] 가 maximize 상태가 아니라서 minimize 제외." );
		}
	}
};
 ...
 * <code>
	{
		target : {String|HTMLElement|dhtmlXWindowsCell}
		, DHTMLX : {
 ...
 * @function
 */
var _stickAll = function()
{
	for( var key in _DHTMLX.windows ) _DHTMLX.windows[ key ].stick();
};
 ...
 * @function
 */
var _unstickAll = function()
{
	for( var key in _DHTMLX.windows ) _DHTMLX.windows[ key ].unstick();
};
...
var _setOptions = function( d )
{
	...
	if( d )
	{
		//_OPTIONS 덮어쓰기 로직 작성;
		d.DHTMLX = d.DHTMLX || {};
		//마지막 입력값으로 전부 초기화 되므로 주석처리 함;
		/*/
		if( d.DHTMLX.width ) _OPTIONS.DHTMLX.width = d.DHTMLX.width;
		if( d.DHTMLX.height ) _OPTIONS.DHTMLX.height = d.DHTMLX.height;

		if( d.DHTMLX.x ) _OPTIONS.DHTMLX.x = d.DHTMLX.x;
		if( d.DHTMLX.y ) _OPTIONS.DHTMLX.y = d.DHTMLX.y;
		//*/
	}
	else
	{
...
(function(){
	var _ = _THIS;
		...
		//_.currentWindow = _THIS.currentWindow


		_.addWindow = _addWindow;

		_.bringToTop = _bringToTop;
		_.closeAll = _closeAll;
		...
		_.hideAll = _hideAll;
		_.maximizeAll = _maximizeAll;
		_.minimizeAll = _minimizeAll;
		...
		_.showAll = _showAll;
		_.stickAll = _stickAll;
		_.unstickAll = _unstickAll;

		_.setData = _setData;
		//_.setParentElement = _setParentElement;

		//_.getCurrentWindow = _getCurrentWindow;
		//_.setCurrentWindow = _setCurrentWindow;
