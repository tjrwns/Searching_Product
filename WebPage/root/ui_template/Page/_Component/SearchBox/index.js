/*/
http://localhost:49781/ui/Page/_Component/SearchBox/index.html?width=200&height=26

http://localhost:49781/ui/Page/_Component/SearchBox/index.html?width=500&height=500

//사이즈를 %로 적용;
http://localhost:49781/ui/Page/_Component/SearchBox/index.html?width=100%25&height=500
http://localhost:49781/ui/Page/_Component/SearchBox/index.html?width=100%25&height=800

//사이즈를 %로 적용 - Height는 적용 안됨;
http://localhost:49781/ui/Page/_Component/SearchBox/index.html?width=100%25&height=100%25

http://localhost:49781/ui/Page/_Component/SearchBox/index.html?url=http://localhost:49781/ui/Page/_Component/SearchBox/index.json
http://dnsproxy.shop:49781/ui/Page/_Component/SearchBox/index.html?url=http://dnsproxy.shop:49781/ui/Page/_Component/SearchBox/index.json
//*/
//----------------------------------------------------------------------------------------------------;
var fileNm = "ui/Page/_Component/SearchBox/index.js";
...
var _IDS = (function(){
	return {
		ROOT : "ui/Page/_Component/SearchBox/index"
	};
})();
...
var _ELS = (function(){
	...
	o.BTN__SEARCHBOX = f0( o.ROOT, "btn__searchbox" );

	o.INPUT__SEARCHBOX = f0( o.ROOT, "input__searchbox" );
...
var _URLS = (function(){
	...
	var o = {
		addKeyword : HOST + "/reqKeyword/crud/addKeyword?keyword=<!=KEYWORD=!>&cd$BRAND=<!=CD$BRAND=!>&cd$COUNTRY_k=<!=CD$COUNTRY_k=!>"
		, addKeyword__allow : HOST + "/reqKeyword/crud/addKeyword__allow?keyword=<!=KEYWORD=!>&cd$BRAND=<!=CD$BRAND=!>&cd$COUNTRY_k=<!=CD$COUNTRY_k=!>"
		, checkAllowKeyword : HOST + "/reqKeyword/validate/checkAllowKeyword?keyword=<!=KEYWORD=!>"
	};
...
var _addEvent = function()
{
	...
	_ELS.BTN__SEARCHBOX.addEventListener( "click", _reqData, false, 0, true );

	_ELS.INPUT__SEARCHBOX.addEventListener( "keyup", _evt_keyup__input_searchbox, false, 0, true );
...
{
	_ELS.BTN__SEARCHBOX.removeEventListener( "click", _reqData, false );

	_ELS.INPUT__SEARCHBOX.removeEventListener( "keyup", _evt_keyup__input_searchbox, false );
};

/**
 * input__searchbox 엔터 처리
 * @function
 * @param event {Event}
 */
var _evt_keyup__input_searchbox = function( event ){ if( event.keyCode != 13 ) return; _reqData(); };
 ...
 */
var _checkKeyword = function( keyword ){
	var url = _URLS.checkAllowKeyword.replace( "<!=KEYWORD=!>", keyword );
	window.b2link_service.common._fn_req( url, _checkKeyword.complete );
};
//검색 허용 키워드 여부의 콜백함수;
_checkKeyword.complete = function( responseData ){
	...
	if( responseData.data.bFind )_reqDataAddKeyword_Allow();//검색허용키워드일 경우;
	else _reqDataAddKeyword();//검색허용키워드가 아닐경우;
...
var _reqData =  function( url, cbComplete ){
	...
	var keyword = _ELS.INPUT__SEARCHBOX.value;
	//허용검색어 여부를 먼저 판단한다;
 ...
 * @param {String} keyword
 */
var _reqDataAddKeyword = function( keyword )
{
	if( !_reqDataAddKeyword.checkKeyword() )
	{
		window.b2link_ui.message.error( "검색어를 확인하시기 바랍니다." );
		return;
	}
	...
	var cd$COUNTRY_k = _ELS.SELECT__COUNTRY.value;
	//----------URI PARAMETERS;

	var url = _URLS.addKeyword.replace( "<!=KEYWORD=!>", keyword )
		.replace( "<!=CD$COUNTRY_k=!>", cd$COUNTRY_k )
		.replace( "<!=CD$BRAND=!>", cd$BRAND );

	window.b2link_service.common._fn_req( url, _reqData.complete );
};
//입력된 키워드 검증;
_reqDataAddKeyword.checkKeyword = function(){
	var _keyword = _ELS.INPUT__SEARCHBOX.value;
	return window.b2link.validate.requestKeyword__SYS0310( _keyword );
};

/**
 * 검색 허용된 키워드를 서버에 요청한다
 * @function
 */
var _reqDataAddKeyword_Allow = function( )
{
	//----------URI PARAMETERS;
	var keyword = _ELS.INPUT__SEARCHBOX.value;
	...
	var url = _URLS.addKeyword__allow.replace( "<!=KEYWORD=!>", keyword )
		.replace( "<!=CD$COUNTRY_k=!>", cd$COUNTRY_k )
		.replace( "<!=CD$BRAND=!>", cd$BRAND );
