/*/
http://localhost:49781/ui/Page/_Component/_Complex/EChart+FancyGrid/index.html?width=500&height=500

//사이즈를 %로 적용;
http://localhost:49781/ui/Page/_Component/_Complex/EChart+FancyGrid/index.html?width=100%25&height=500
http://localhost:49781/ui/Page/_Component/_Complex/EChart+FancyGrid/index.html?width=100%25&height=800

//사이즈를 %로 적용 - Height는 적용 안됨;
http://localhost:49781/ui/Page/_Component/_Complex/EChart+FancyGrid/index.html?width=100%25&height=100%25

http://localhost:49781/ui/Page/_Component/_Complex/EChart+FancyGrid/index.html?url=http://localhost:49781/ui/Page/_Component/_Complex/EChart+FancyGrid/index.json
http://dnsproxy.shop:49781/ui/Page/_Component/_Complex/EChart+FancyGrid/index.html?url=http://dnsproxy.shop:49781/ui/Page/_Component/_Complex/EChart+FancyGrid/index.json
//*/
//----------------------------------------------------------------------------------------------------;
var fileNm = "ui/Page/_Component/_Complex/EChart+FancyGrid/index.js";
...
var _IDS = (function(){
	return {
		ROOT : "ui/Page/_Component/_Complex/EChart+FancyGrid/index"
	};
})();
...
var _ELS = (function(){
	...
	o.ECHART = f0( o.ROOT, "ui/Page/_Component/_Complex/EChart+FancyGrid/index--EChart" );
	o.FANCYGRID = f0( o.ROOT, "ui/Page/_Component/_Complex/EChart+FancyGrid/index--Fancygrid" );
 ...
 * @param {Event}
 */
var _evt_mClick__cell = function( grid, o )
{
	window.b2link.event.dispatchCE__LIST_SELECTED_ITEM( _ELS.FANCYGRID, o );
};
...
function initialize()
{
	try
	{
		...
		var els_fg = window.SYS0310_UI_API.fancygrid.addBasic();

		_FANCYGRID.ELEMENT = els_fg[ 0 ];
		_FANCYGRID.CTRL = els_fg[ 1 ];

		_addFancygrid( _FANCYGRID.ELEMENT );
 ...
 * @param {HTMLElement} d
 */
var _addEChart = function( d )
{
	window.b2link.element.removeAllChildren( _ELS.ECHART );
	_ELS.ECHART.appendChild( d );
};
 ...
 * @param {HTMLElement} d
 */
var _addFancygrid = function( d )
{
	window.b2link.element.removeAllChildren( _ELS.FANCYGRID );
	_ELS.FANCYGRID.appendChild( d );
};
 ...
 * @return {Object} r
 */
var _copyDataFancygridToEChart = function( target, sub )
{
	var d = JSON.parse( JSON.stringify( _FANCYGRID.CTRL.getData() ) );

	if( !d.columns || !d.data || d.columns.length < 1 || d.data.length < 1 )
	{
		debugger;
		return ;
	}
	...
	for( ; i<iLen; ++i )
	{
		...
		if( io.type == "number" )
		{
			if( target && target == io.index )
			{
				r.legend = [];
				r.legend.push( io.index );
				if( !sub ) target = d.columns[ 0 ].index;
				else target = sub;
				break;
			}
			else
		...
		if( !target && i == 0 ) target = io.index;
	}

	// data 설정;
	var i=0,iLen=r.legend.length;
	var io;
	for( ; i<iLen; ++i )
	{
		io = r.legend[ i ];

		var j=0,jLen=d.data.length;
		var jo;
		var arr_data = [];
		for( ; j<jLen; ++j )
		{
			...
			if( 0 == i ) r.axis.push( jo[ target ] )
		}

		r.data.push( arr_data );
	}
...
var _draw = function( data, options )
{
	...
	//그리기 위한 로직 추가;
	_FANCYGRID.CTRL.draw( data, options );
};
...
var _redraw = function( data, options )
{
	...
	// _draw( data, options );
	_FANCYGRID.CTRL.redraw( data, options );
};
 ...
 * @param {number} idx
 */
var _selectRow = function( idx )
{
	_FANCYGRID.GRID.selectRow( idx, false );
};
...
/**
 * @function
 * @return {Object} Fancygrid 데이터 가져오기
 */
var _getData = function()
{
	return _FANCYGRID.CTRL.getData();
};
...
var _setMode = function( mode ){
	if( "Search" == mode )
	{
		if( _FANCYGRID.CTRL )
		{
			_FANCYGRID.CTRL.dispose();

			var els_fg = window.SYS0310_UI_API.fancygrid.addSearch();
...
{
	_FANCYGRID.CTRL.setParentElement( parentElement );
};
...
(function(){
	var _ = _THIS;
		...
		_.addEChart = _addEChart;
		_.copyDataFancygridToEChart = _copyDataFancygridToEChart;
