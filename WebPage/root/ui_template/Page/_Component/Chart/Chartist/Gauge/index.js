/*/
http://localhost:49781/ui/Page/_Component/Chart/Chartist/Gauge/index.html?width=500&height=500

//사이즈를 %로 적용;
http://localhost:49781/ui/Page/_Component/Chart/Chartist/Gauge/index.html?width=100%25&height=500
http://localhost:49781/ui/Page/_Component/Chart/Chartist/Gauge/index.html?width=100%25&height=800

//사이즈를 %로 적용 - Height는 적용 안됨;
http://localhost:49781/ui/Page/_Component/Chart/Chartist/Gauge/index.html?width=100%25&height=100%25

http://localhost:49781/ui/Page/_Component/Chart/Chartist/Gauge/index.html?url=http://localhost:49781/ui/Page/_Component/Chart/Chartist/Gauge/index.json
http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Chartist/Gauge/index.html?url=http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Chartist/Gauge/index.json
//*/
//----------------------------------------------------------------------------------------------------;
var fileNm = "ui/Page/_Component/Chart/Chartist/Gauge/index.js";
...
var _IDS = (function(){
	return {
		ROOT : "ui/Page/_Component/Chart/Chartist/Gauge/index"
	};
})();
...
var _DATA = (function(){
	...
	return {
		data : { series: [ 10, 20, 40, 30 ] }
		//data : { series : [ { value : 20, name : "Series 1", className : "my-custom-class-one", meta : "Meta One" }, { value : 10, name : "Series 2", className : "my-custom-class-two", meta : "Meta Two" }, { value : 70, name : "Series 3", className : "my-custom-class-three", meta : "Meta Three" } ] }
		//data : { series : [ { value : 20, name : "Series 1", meta : "Meta One" }, { value : 10, name : "Series 2", meta : "Meta Two" }, { value : 70, name : "Series 3", meta : "Meta Three" } ] }
		, width : ( d.width || 500 )
		, height : ( d.height || 500 )
		, donut : true
		, donutWidth : 60
...
var _OPTIONS = {
	...
	, startAngle : 270
	, showLabel : true
	, labelInterpolationFnc : function( value ){ return Math.round( value / _DATA.data.series.reduce( function( a, b ){ return a + b; } ) * 100 ) + '%'; }
};
...
var _createOptions = function( data )
{
	if( !data.data )
	{
		window.TtwLog.error( "[ ERROR ] - _createOptions : data 가 불충분함" );
		return;
	}

	//_OPTIONS 데이터 생성 또는 가감 로직 추가;
	if( data.width ) _OPTIONS.width = data.width;
	if( data.height ) _OPTIONS.height = data.height;
	if( data.hasOwnProperty( "donutWidth" ) ) _OPTIONS.donutWidth = data.donutWidth;
	if( data.startAngle ) _OPTIONS.startAngle = data.startAngle;
	if( data.total ) _OPTIONS.total = data.total;
	if( data.hasOwnProperty( "showLabel" ) ) _OPTIONS.showLabel = data.showLabel;
	//if( data.responsive ) _OPTIONS.responsive = data.responsive;//옵션값이 아님. 실행 시 별도 파라메터로 사용;
};
...
var _draw = function( data, options )
{
	...
	//그리기;
	if( !_CHARTIST.CHART ) _CHARTIST.CHART = new Chartist.Pie( _ELS.ROOT, data.data, options, options_responsive );
...
var _setOptions = function( options )
{
	...
	if( options )
	{
		...
		if( options.hasOwnProperty( "donutWidth" ) ) _OPTIONS.donutWidth = options.donutWidth;
		if( options.startAngle ) _OPTIONS.startAngle = options.startAngle;
		if( options.total ) _OPTIONS.total = options.total;
		if( options.hasOwnProperty( "showLabel" ) ) _OPTIONS.showLabel = options.showLabel;
