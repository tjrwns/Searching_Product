/*/
http://localhost:49781/ui/Page/_Component/Chart/EChart/Radar/index.html?width=500&height=500

//사이즈를 %로 적용;
http://localhost:49781/ui/Page/_Component/Chart/EChart/Radar/index.html?width=100%25&height=500
http://localhost:49781/ui/Page/_Component/Chart/EChart/Radar/index.html?width=100%25&height=800

//사이즈를 %로 적용 - Height는 적용 안됨;
http://localhost:49781/ui/Page/_Component/Chart/EChart/Radar/index.html?width=100%25&height=100%25&

http://localhost:49781/ui/Page/_Component/Chart/EChart/Radar/index.html?url=http://localhost:49781/ui/Page/_Component/Chart/EChart/Radar/index.json
http://dnsproxy.shop:49781/ui/Page/_Component/Chart/EChart/Radar/index.html?url=http://dnsproxy.shop:49780/country/brand
//*/
//----------------------------------------------------------------------------------------------------;
var fileNm = "ui/Page/_Component/Chart/EChart/Radar/index.js";
...
var _IDS = (function(){
	return {
		ROOT : "ui/Page/_Component/Chart/EChart/Radar/index"
	};
})();
...
var _DATA = (function(){
	...
	return {
		title : "Title"
		, axis : [ "销售（sales）", "管理（Administration）", "信息技术（Information Techology）", "客服（Customer Support）", "研发（Development）", "市场（Marketing）" ]
...
var _OPTIONS = {
	title : { text : "" }
	, legend : { data : null }
	, radar : { name : { textStyle : { color : "#fff", backgroundColor : "#999", borderRadius : 3, padding : [3, 5] } }, indicator : null }
	, series : null
	, tooltip : {}
};
...
var _createOptions = function( d )
{
	...
	for( ; i<iLen; ++i )
	{
		...
		if( !o.data ) o.data = [];
		o.data.push( { value : io, name : d.legend[ i ] } );

		var j=0,jLen=io.length;
		var obj_max = {};
		for( ; j<jLen; ++j )
		{
			if( !obj_max[ d.axis[ j ] ] ) obj_max[ d.axis[ j ] ] = {};
			obj_max[ d.axis[ j ] ].name = d.axis[ j ];

			if( !obj_max[ d.axis[ j ] ].max ) obj_max[ d.axis[ j ] ].max = 0;
			if( io[ j ] > obj_max[ d.axis[ j ] ].max ) obj_max[ d.axis[ j ] ].max = io[ j ];
		}
		arr_max.push( obj_max );
	}
	series.push( o );

	var i=0, iLen=arr_max.length;
	var io;
	for( ; i<iLen; ++i )
	{
		io = arr_max[ i ];
		...
		for( var s in io )
		{
			if( !obj_axis[ s ] ) obj_axis[ s ] = {};
			obj_axis[ s ].name = io[ s ].name;
			if( !obj_axis[ s ].max ) obj_axis[ s ].max = 0;
			if( io[ s ].max > obj_axis[ s ].max ) obj_axis[ s ].max = io[ s ].max;
		}
	}

	for( var s in obj_axis ) arr_axis.push( obj_axis[ s ] );


	//chart option 설정;
	if( d.title ) _OPTIONS.title.text = d.title;//chart option - 타이틀 설정;
	_OPTIONS.legend.data = d.legend;//chart option - 레전드 설정;
	_OPTIONS.radar.indicator = arr_axis;//chart option - x축 설정;
 ...
 * @example
 * <code>
 	var els = window.SYS0310_UI_API.echart.addRadar();
		els[ 1 ].setData({ url : "http://localhost:49781/ui/Page/_Component/Chart/EChart/Radar/index.json" });
		els[ 1 ].redraw();
 * </code>
 */
(function(){
