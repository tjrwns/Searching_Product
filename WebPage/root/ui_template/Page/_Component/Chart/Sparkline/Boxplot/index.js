/*/
http://localhost:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html
http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html

//사이즈 설정;
http://localhost:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?width=500&height=500
http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?width=500&height=500

//사이즈를 %로 적용;
http://localhost:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?width=100%25&height=500
http://localhost:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?width=100%25&height=800
http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?width=100%25&height=500
http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?width=100%25&height=800

//사이즈를 %로 적용 - Height는 적용 안됨;
http://localhost:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?width=100%25&height=100%25&
http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?width=100%25&height=100%25&


http://localhost:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?url=http://localhost:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.json
http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.html?url=http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.json
//*/
//----------------------------------------------------------------------------------------------------;
var fileNm = "ui/Page/_Component/Chart/Sparkline/Boxplot/index.js";
...
var _IDS = (function(){
	return {
		ROOT : "ui/Page/_Component/Chart/Sparkline/Boxplot/index"
	};
})();
...
var _OPTIONS = {
	...
	type : "box"
	//Width of the chart - Defaults to 'auto' - May be any valid css width - 1.5em, 20px, etc (using a number without a unit specifier won't do what you want) - This option does nothing for Boxplot and tristate chars (see BoxplotWidth)
	//차트 너비-기본값은 'auto'-유효한 CSS 너비-1.5em, 20px 등일 수 있습니다 (단위 지정자가없는 숫자를 사용하면 원하는대로 수행되지 않음)-이 옵션은 막대 및 삼 상태 문자에 대해서는 아무 것도 수행하지 않습니다. (BoxplotWidth 참조);
	, width	: "100%"
	...
	//false (기본값)로 설정하면 제공된 값을 사용하여 박스 데이터 포인트를 계산합니다. 참이면 포인트를 미리 계산해야합니다 (아래 참조).;
	, raw : false
	...
	//최소값과 최대 값을 설정하면 플롯의 스케일이 고정됩니다. 기본적으로 minValue 및 maxValue는 제공된 값에서 추론됩니다.;
	, minValue : null
	//See minValue;
	//minValue 참조;
	, maxValue : null
...
var _setOptions = function( d )
{
	...
	if( d )
	{
		...
		//Boxplot OPTIONS ----------;
		if( d.raw ) _OPTIONS.raw = d.raw;
		if( d.showOutliers ) _OPTIONS.showOutliers = d.showOutliers;
		if( d.outlierIQR ) _OPTIONS.outlierIQR = d.outlierIQR;
		if( d.boxLineColor ) _OPTIONS.boxLineColor = d.boxLineColor;
		if( d.boxFillColor ) _OPTIONS.boxFillColor = d.boxFillColor;
		if( d.whiskerColor ) _OPTIONS.whiskerColor = d.whiskerColor;
		if( d.outlierLineColor ) _OPTIONS.outlierLineColor = d.outlierLineColor;
		if( d.outlierFillColor ) _OPTIONS.outlierFillColor = d.outlierFillColor;
		if( d.spotRadius ) _OPTIONS.spotRadius = d.spotRadius;
		if( d.medianColor ) _OPTIONS.medianColor = d.medianColor;
		if( d.target ) _OPTIONS.target = d.target;
		if( d.targetColor ) _OPTIONS.targetColor = d.targetColor;
		if( d.minValue ) _OPTIONS.minValue = d.minValue;
		if( d.maxValue ) _OPTIONS.maxValue = d.maxValue;
	}
	else
	{
	}
...
/**
 * Chart - Sparkline Boxplot
 * @example
 * <code>
 	var els = window.SYS0310_UI_API.sparkline.addBoxplot();
		els[ 1 ].setData({ url : "http://localhost:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.json" });
		//els[ 1 ].setData({ url : "http://dnsproxy.shop:49781/ui/Page/_Component/Chart/Sparkline/Boxplot/index.json" });
